...
class AppStore extends Component {
  state = {activateTabId: tabsList[0].tabId, searchInput: ''}
  ...
  getFilteredApps = (searchResults, activateTabId) => {
  ...
  render() {
    const {searchInput, activateTabId} = this.state

    const searchResults = this.getSearchResults(searchInput)
    const filteredApps = this.getFilteredApps(searchResults, activateTabId)
          ...
          <ul className="tabs-list-container">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                tabDetails={eachTab}
                updateActivateTabId={this.updateActivateTabId}
                isActive={eachTab.tabId === activateTabId}
              />
            ))}
          </ul>
